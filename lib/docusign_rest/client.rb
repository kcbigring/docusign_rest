...
module DocusignRest
  ...
  class Client
    ...
    # email_settings- Optional hash of settings to override the default
    #                 Reply To settings.
    #
    ...
    def create_envelope_from_document(options={})
      ...
      post_body = {
        ...
        recipients: {
          ...
          editors: options[:editors] ? get_signers(options[:editors]) : nil,
          carbonCopies: options[:carbon_copies] ? get_signers(options[:carbon_copies]) : nil,
        },
        status: "#{options[:status]}",
        emailSettings: get_email_override_settings(options[:email_settings])
      }.to_json
    ...
    # Internal: takes a hash for email settings and parses it into the
    # correct keys.
    #
    # reply_email_address_override - Address of the email to replace as the
    #                                default Reply To of the envelope.
    # reply_email_name_override    - Name to be associated with the Reply To
    #                                address.
    #
